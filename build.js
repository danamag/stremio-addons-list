    ...
    getPosts().then(data => {
      ...
      const addDataForAddon = (listAddonHtml, body, task, labelsAreLinks) => {
        ...
        if (!(body.behaviorHints || {}).configurationRequired)
          addonHtml = addonHtml.replace('{install-button}', '<a class="addon-button install-button" href="'+task.url.replace('https://','stremio://')+'">Install</a><a class="addon-button copy-link-button" href="#" onClick="copyLink.bind(null, \''+task.url+'\')">Copy Link</a>')
        else
